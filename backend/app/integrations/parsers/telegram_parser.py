...
def extract_event_with_gigachat(text: str, token: str) -> dict:
    ...
    ВАЖНОЕ ПРАВИЛО: Сохраняй мероприятие ТОЛЬКО если оно жестко связано с IT (программирование, разработка, дизайн, хакатоны, аналитика, ИИ) И проходит в городе Екатеринбург (либо онлайн). Если это не IT или другой город — верни null во всех полях.
    Верни СТРОГО один JSON объект (не массив). Без markdown, без лишних слов.
    1. "title": Короткое название мероприятия (Если не IT/не Екб или новость - null).
    2. "event_date": Дата ISO 8601 (например "2026-06-15"). Если даты нет - null.
