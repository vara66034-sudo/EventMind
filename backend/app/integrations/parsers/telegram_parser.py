...
def extract_event_with_gigachat(text: str, token: str) -> dict:
    ...
    # ОБНОВЛЕННЫЙ ПРОМПТ (Добавлено description, изменено location)
    ...
    payload = json.dumps(
        {"model": "GigaChat-Max", "messages": [{"role": "user", "content": prompt}], "temperature": 0.1})

    try:
        response = requests.post(url, headers=headers, data=payload, verify=False)
        raw_content = response.json()['choices'][0]['message']['content'].replace("```json", "").replace("```",
                                                                                                         "").strip()
...
async def fetch_telegram_posts():
    ...
    try:
            ...
            if message.text:
                ...
                if message.photo:
                    image_path = await client.download_media(message.photo,
                                                             file=f"images/{CHANNEL_IT_ONLINE}_{message.id}.jpg")
        ...
        print(f"⚠️ Ошибка {CHANNEL_IT_ONLINE}: {e}")
            ...
            async for message in client.iter_messages(entity, limit=30):
                if message.text:
                    ...
                    if message.photo:
                        ...
                        image_path = await client.download_media(message.photo,
                                                                 file=f"images/{clean_channel_name}_{message.id}.jpg")
...
for post in posts_to_process:
    ...
    # === ОБНОВЛЕННЫЕ КЛЮЧИ ДЛЯ ФИНАЛЬНОГО JSON ===
    final_events.append({
        "source": "telegram",  # Платформа-источник (для базы данных)
        "source_url": post["source"],  # Сама ссылка на пост
        "title": title,
